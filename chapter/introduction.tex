...
% presenta il layer che migliora dingnet aggiungendo il supporto per la parte applicativa dentro lorawan e il lavoro che è stato fatto per consentire a protelis di eseguire su una rete LoRaWAN
\Cref{chap:contribution} presents the connection layer, which improve DingNet adding the support to execute applications over LoRaWAN, and the work done to allow the execution of Protelis application over the LoRaWAN network.
%  
\Cref{chap:case-staudyLoRa} and \Cref{chap:case-staudyAC} introduce two case-studies developed in the new simulation framework. \todo{modify when make only 1 chapter}
% 
Finally, \Cref{chap:conclusions} concludes this thesis by summarising its main contribution and presenting future works and interesting topic to evaluate.